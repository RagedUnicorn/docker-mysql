...
function set_init_done {
  touch "${MYSQL_DATA_DIR}/.init"
  echo "$(date) [INFO]: Init script done"
}

function init {
  if [ -f "${MYSQL_DATA_DIR}/.init" ]; then
    ...
    if [ -f "${mysql_app_user}" ] && [ -f "${mysql_app_password}" ] && [ -f "${mysql_root_password}" ]; then
      ...
      mysql_root_password="${MYSQL_ROOT_PASSWORD:?Missing environment variable MYSQL_ROOT_PASSWORD}"
